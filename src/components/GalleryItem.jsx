...
const GalleryItem = ({ src, alt, index, handelCLick, title }) => {
  ...
  return (
    <div className="relative cursor-pointer wrapper-images aspect-square flex items-center justify-center">
      {isLoading ? <GalleryLoader /> : <img src={imageURL + src} alt={alt} className="aspect-square object-cover w-full fade-in" />}

      <div className="absolute bottom-0 left-0 w-full h-full bg-black bg-opacity-60 text-white p-4 grid place-items-center opacity-0 hover:opacity-100 transition-all" onClick={() => handelCLick(src, index, title)}>
